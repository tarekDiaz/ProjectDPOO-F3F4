...
public class Aventurer extends Personatge{
    ...
    public void evolucionarPersonatge() {
        if (getNivell() > 3 && getNivell() < 8) {
            Guerrer evolucio = new Guerrer(getNom(), getNomJugador(), getNivell(), getCos(), getMent(), getEsperit(), getClasse(), getExperiencia(), getPdvMax(), getPdvActual(), getIniciativa(), getTipusDeMal());
        }
        if (getNivell() > 7) {
